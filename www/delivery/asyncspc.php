...
. $newImg . '" style="width:auto;height:auto;"><div style="width:100%;display:block;height:50px;"><div class="ptitle">' 
. $newTitle . '</div><div class="pprice">' 
. $newPrice . '</div></div></div>';
$code = str_replace($origImg, $newDiv, $newString);
$origTgt = " target='_blank'";
$newTgt = " target='_blank' rel='nofollow'";
$code = str_replace($origTgt, $newTgt, $code);
	return $code;
}
...
,az.zone_id
,c.revenue,c.ecpm_enabled, c.ecpm
...
revive.rv_campaigns c on c.campaignid = b.campaignid
	...
	while ($aAdsInfo = OA_Dal_Delivery_fetchAssoc($rAdsInfo)) {
		array_push($aAds, $aAdsInfo);
	}
	foreach ($aAds as $aId => $ad){
		if (!MAX_limitationsCheckAcl($ad)) {
			unset($aAds[$aId]);
		}
	}
	return $aAds;
