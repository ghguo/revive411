							...
							if (o) {
								...
								if (r == 3) {
									...
									for (let hh = 0; hh < kk.length; hh++) {
										let s = kk[hh].split("=");
										tt.push(encodeURIComponent(s[0]) + "=" + (s[0] == "name" ? s[1] : encodeURIComponent(s[1])))
									}
											...
											if (200 === this.status) {
												...
												showprod = function () {
													if (window.adr_idx < window.adr_ps.length) {
														...
														window.adr_idx += 1;
														window.adr_o3.innerHTML = '<a href="' + ps[2] + '" rel="nofollow" target="_blank"><div style="display:inline-block;text-align:center"><img src="' + ps[3] + '" style="width:auto;height:auto;margin-left:auto;margin-right:auto;" /><div><div style="padding-top:1px">' + ps[0] + '</div><div style="padding-top:1px">' + (ps[1] == null ? ' ' : ps[1]) + '</div></div></div></a>'
													} else {
														window.adr_idx = 0
													}
												};
