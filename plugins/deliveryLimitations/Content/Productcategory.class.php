 ...
 * basis of the product category of viewed content.
 */
class Plugins_DeliveryLimitations_Content_Productcategory extends Plugins_DeliveryLimitations
{
    var $delimiter = ',';
    ...
    function __construct()
    {
		...
		$this->nameEnglish = 'Content - Product Category';
    }
    ...
    function displayData()
    {
		...
		echo "<select class='pcat1' tabindex='".($tabindex++)."' id='c_{$this->executionorder}_1' name='acl[{$this->executionorder}][data][]'>";
		$catLevl = $GLOBALS['PRODUCT_CATEGORIES']['LEVEL1'];
		...
		echo "<br>";
		echo "<select class='pcat2' tabindex='".($tabindex++)."' id='c_{$this->executionorder}_2' name='acl[{$this->executionorder}][data][]'>";
		$catLevl = $GLOBALS['PRODUCT_CATEGORIES']['LEVEL2'];
		foreach ($catLevl as $key => $val) {
			echo "<option value='{$key}'" . (in_array($key, $this->data) ? " selected='selected'" : "") . ">" . $val . "</option>";
		}
		echo "</select>";
